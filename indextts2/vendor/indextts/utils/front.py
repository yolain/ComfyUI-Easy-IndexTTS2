...
class TextNormalizer:
    ...
    def load(self):
        # Initialize text normalizers with robust fallbacks across platforms
        if self.zh_normalizer is not None and self.en_normalizer is not None:
            return
        # 1) Try wetext first (often available on macOS/Windows)
        try:
        ...
        # 3) Final fallback: minimal pass-through normalizer to avoid hard crash
        class _DummyNormalizer:
            def __init__(self, lang="zh", operator="tn", **kwargs):
                pass
        ...
        warnings.warn(
            "Text normalizer dependencies not found; falling back to a minimal normalizer.")
        self.zh_normalizer = _DummyNormalizer()
        self.en_normalizer = _DummyNormalizer(lang="en")
